// Castle Age Monster Kill Time Estimator
// version 0.3 ALPHA!
// 2010-09-11
...
//
// ==UserScript==
// @name          Castle Age Monster Kill Time Estimator
    ...
    if (drgContainer.length > 0) {
        ...
        if (drgContainer.length == 1) {
            for (var i = 0; imgs.length > i; i++) {
                // Get Health
                if (imgs[i].src.search(/nm_red\.jpg$/) !== -1 || imgs[i].src.search(/monster_health_background\.jpg/) !== -1) {
                    health = imgs[i].parentNode.style.width
                }
                // Get Party Strength / Defense
                else if (imgs[i].src.search(/nm_green\.jpg$/) !== -1) {
                    strength = imgs[i].parentNode.style.width.replace("%", "");
                }
                else if (imgs[i].src.search(/bar_dispel\.gif$/) !== -1) {
                    strength = 100 - parseInt(imgs[i].parentNode.style.width.replace("%", ""));
                }
            }
        ...
        // Format Results
        var data = "&nbsp;&nbsp;<b>Monster Health</b> " + Math.round(health.split("%")[0]) + "%<br/>" + "&nbsp;&nbsp;<b>Party Strength</b> " + Math.round(strength) + "%<br/><br/>" +

        "&nbsp;&nbsp;<b>DPS</b> " + dps.toString().split(".")[0] + "<br/>" + "&nbsp;&nbsp;<b>Total Damage</b> " + addCommas(dmg) + "<br/><br/>" +

        "&nbsp;&nbsp;<b>Remaining</b> " + document.getElementById("app46755028429_monsterTicker").textContent + "<br/>" + "&nbsp;&nbsp;<b>ETD</b> " + makeTimer((eta - Date.now()) / 1000);
...
var indent = "&nbsp;&nbsp;"
desc = "<h4>Monster Death Estimator</h4><br/><span id='ca-monster-data'><a href='javascript:void(0);' id='pollMonster' title='Check Monster'>Check Monster</a></span>";
div = document.createElement('div');
div.id = 'ca-etd-cpanel';
div.innerHTML = desc;
document.body.style.paddingBottom = "4em";
document.body.appendChild(div);
var btn = document.getElementById("pollMonster")
btn.addEventListener("click", function () { pollPage(); }, true);